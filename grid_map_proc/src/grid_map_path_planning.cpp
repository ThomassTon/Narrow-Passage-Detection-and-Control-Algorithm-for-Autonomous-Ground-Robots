...
namespace grid_map_path_planning{
                                    ...
                                    const std::string expl_trans_layer)
  {
    ...
    bool findPathExplorationTransform_2(grid_map::GridMap& grid_map,
                                    const geometry_msgs::Pose& start_pose,
                                    std::vector<geometry_msgs::PoseStamped>& path,
                                    float* path_cost,
                                    const std::string occupancy_layer,
                                    ...
                                    const std::string expl_trans_layer)
  {
    ...
    for (size_t i = 0; i < path_indices.size(); ++i){
      ...
      if (i < (path_indices.size()-1)){
        ...
        msg.pose.orientation.z = sin(yaw*0.5f);
        msg.pose.orientation.w = cos(yaw*0.5f);
        path.push_back(msg);
      }else{
        const geometry_msgs::Pose& prior_pose = path[i-1].pose;
        
        msg.pose.orientation = prior_pose.orientation;
      }
