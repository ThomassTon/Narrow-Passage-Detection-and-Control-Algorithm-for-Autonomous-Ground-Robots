...
bool addInflatedLayer( grid_map::GridMap &grid_map, const float inflation_radius_map_cells,
                       const std::string occupancy_layer, const std::string inflated_occupancy_layer )
{
  ...
  cv::Mat element =
      cv::getStructuringElement( erosion_type, cv::Size( 2 * erosion_size + 1, 2 * erosion_size + 1 ),
...
bool addDeflatedLayer( grid_map::GridMap &grid_map, const float deflation_radius_map_cells,
                       const std::string occupancy_layer, const std::string deflated_occupancy_layer )
{
  if ( !grid_map.exists( occupancy_layer ) )
    return false;
...
bool addDistanceTransformCv( grid_map::GridMap &grid_map, const std::string occupancy_layer,
...
bool addDistanceTransform( grid_map::GridMap &grid_map, const grid_map::Index &seed_point,
                           std::vector<grid_map::Index> &obstacle_cells,
                           std::vector<grid_map::Index> &frontier_cells, float min_frontier_dist_,
                           int min_frontier_size, const std::string occupancy_layer,
                           const std::string dist_trans_layer )
{
  if ( !grid_map.exists( occupancy_layer ) )
    return false;
  ...
  collectReachableObstacleCells( grid_map, seed_point, obstacle_cells, frontier_cells,
                                 min_frontier_dist_, min_frontier_size, occupancy_layer );
  ...
  while ( point_queue.size() ) {
    ...
    if ( point( 0 ) < 1 || point( 0 ) >= size_x_lim || point( 1 ) < 1 || point( 1 ) >= size_y_lim ) {
      continue;
    }

    float current_val = dist_layer( point( 0 ), point( 1 ) );

    touchDistCell( grid_data, dist_layer, point( 0 ) - 1, point( 1 ) - 1, current_val,
                   diagonal_dist, point_queue );

    touchDistCell( grid_data, dist_layer, point( 0 ), point( 1 ) - 1, current_val, adjacent_dist,
                   point_queue );

    touchDistCell( grid_data, dist_layer, point( 0 ) + 1, point( 1 ) - 1, current_val,
                   diagonal_dist, point_queue );

    touchDistCell( grid_data, dist_layer, point( 0 ) - 1, point( 1 ), current_val, adjacent_dist,
                   point_queue );
...
bool addExplorationTransform( grid_map::GridMap &grid_map,
                              ...
                              const float lethal_dist, const float penalty_dist,
                              const float penalty_weight, const std::string occupancy_layer,
                              const std::string dist_trans_layer, const std::string expl_trans_layer )
{
  ...
  for ( size_t i = 0; i < goal_points.size(); ++i ) { // set goal value=0
  ...
  while ( point_queue.size() ) {
    ...
    touchExplorationCell( grid_data, dist_data, expl_layer, point( 0 ) - 1, point( 1 ) - 1,
                          current_val, diagonal_dist, lethal_dist, penalty_dist, penalty_weight,
                          point_queue );

    touchExplorationCell( grid_data, dist_data, expl_layer, point( 0 ), point( 1 ) - 1, current_val,
                          adjacent_dist, lethal_dist, penalty_dist, penalty_weight, point_queue );

    touchExplorationCell( grid_data, dist_data, expl_layer, point( 0 ) + 1, point( 1 ) - 1,
                          current_val, diagonal_dist, lethal_dist, penalty_dist, penalty_weight,
                          point_queue );

    touchExplorationCell( grid_data, dist_data, expl_layer, point( 0 ) - 1, point( 1 ), current_val,
                          adjacent_dist, lethal_dist, penalty_dist, penalty_weight, point_queue );
...
bool addExplorationTransform_narrow( grid_map::GridMap &grid_map,
                              const std::vector<grid_map::Index> &goal_points,
                              float lethal_dist, float penalty_dist,
                              float penalty_weight, const std::string occupancy_layer,
                              const std::string dist_trans_layer, const std::string expl_trans_layer )
{
  if ( !grid_map.exists( occupancy_layer ) )
    return false;
  ...
  // float adjacent_dist = 1.3693;
  // float diagonal_dist = 0.955;
  ...
  while ( point_queue.size() ) {
    ...
    touchExplorationCell_narrow( grid_data, dist_data, expl_layer, point( 0 ) - 1, point( 1 ) - 1,
                          current_val, diagonal_dist, lethal_dist, penalty_dist, penalty_weight,
                          point_queue );

    touchExplorationCell_narrow( grid_data, dist_data, expl_layer, point( 0 ), point( 1 ) - 1, current_val,
                          adjacent_dist, lethal_dist, penalty_dist, penalty_weight, point_queue );
    ...
    touchExplorationCell_narrow( grid_data, dist_data, expl_layer, point( 0 ) - 1, point( 1 ), current_val,
                          adjacent_dist, lethal_dist, penalty_dist, penalty_weight, point_queue );
...
bool collectReachableObstacleCells( grid_map::GridMap &grid_map, const grid_map::Index &seed_point,
                                    std::vector<grid_map::Index> &obstacle_cells,
                                    std::vector<grid_map::Index> &frontier_cells,
                                    float min_frontier_dist, int min_frontier_size,
                                    ...
                                    const std::string dist_seed_layer )
{
  ...
  while ( point_queue.size() ) {
    ...
    touchObstacleSearchCell( grid_data, expl_layer, resolution, seed_point, point, point( 0 ) - 1,
                             point( 1 ) - 1, obstacle_cells, frontier_cells, point_queue,
                             min_distance_sq );

    touchObstacleSearchCell( grid_data, expl_layer, resolution, seed_point, point, point( 0 ),
                             point( 1 ) - 1, obstacle_cells, frontier_cells, point_queue,
                             min_distance_sq );
    ...
    touchObstacleSearchCell( grid_data, expl_layer, resolution, seed_point, point, point( 0 ) - 1,
                             point( 1 ), obstacle_cells, frontier_cells, point_queue,
                             min_distance_sq );
...
void touchExplorationCell( const grid_map::Matrix &grid_map, const grid_map::Matrix &dist_map,
                           grid_map::Matrix &expl_trans_map, const int idx_x, const int idx_y,
                           const float curr_val, const float add_cost, const float lethal_dist,
                           const float penalty_dist, const float penalty_weight,
                           std::queue<grid_map::Index> &point_queue )
{
  ...
  // }
  double penalty_dist_ = penalty_dist;
  if ( dist < penalty_dist_) { // near to obstcal should have a penalty     penalty_dist
    float add_cost = ( penalty_dist_ - dist );
    cost +=  penalty_weight* ( add_cost * add_cost); // 0.0005    penalty_dist
...
void touchExplorationCell_narrow( const grid_map::Matrix &grid_map, const grid_map::Matrix &dist_map,
                                  grid_map::Matrix &expl_trans_map, const int idx_x,
                                  const int idx_y, const float curr_val, const float add_cost,
                                  float lethal_dist, float penalty_dist, float penalty_weight,  //float lethal_dist=5.0, float penalty_dist=6.5, float penalty_weight=1.0,
                                  std::queue<grid_map::Index> &point_queue )
{
  // If not free at cell, return right away
  if ( grid_map( idx_x, idx_y ) != 0 )
...
void touchExplorationCell_narrow_2( const grid_map::Matrix &grid_map, const grid_map::Matrix &dist_map,
                                  grid_map::Matrix &expl_trans_map, const int idx_x,
                                  const int idx_y, const float curr_val, const float add_cost,
                                  float lethal_dist, float penalty_dist, float penalty_weight,
                                  std::queue<grid_map::Index> &point_queue )
{
  // If not free at cell, return right away
  if ( grid_map( idx_x, idx_y ) != 0 )
  ...
  if ( dist < penalty_dist ) { // near to obstcal should have a penalty     penalty_dist
    float add_cost = ( penalty_dist - dist );
    cost += penalty_weight * ( add_cost * add_cost * add_cost * add_cost ); // 0.0005    penalty_weight
    // std::cout<<"penalty_weight:   "<<penalty_weight<<"\n\n\n\n\n\n\n\n";
  }
...
void touchDistCell( const grid_map::Matrix &grid_map, grid_map::Matrix &expl_trans_map,
                    const int idx_x, const int idx_y, const float curr_val, const float add_cost,
                    std::queue<grid_map::Index> &point_queue )
{
  // If not free at cell, return right away
  if ( grid_map( idx_x, idx_y ) != 0 )
...
void touchObstacleSearchCell( const grid_map::Matrix &grid_map, grid_map::Matrix &expl_trans_map,
                              float resolution, const grid_map::Index &start_point,
                              const grid_map::Index &current_point, const int idx_x,
                              const int idx_y, std::vector<grid_map::Index> &obstacle_cells,
                              std::vector<grid_map::Index> &frontier_cells,
                              std::queue<grid_map::Index> &point_queue, float min_distance_sq )
{
  // Free
                                                      ...
                                                      int min_frontier_size )
{
  ...
  for ( auto &frontier_cell : frontier_cells ) {
    ...
    touchFilterCell( expl_trans_map, frontier_cell, point_queue, frontier_cell_cluster );
    while ( !point_queue.empty() ) {
      ...
      for ( int i = -1; i <= 1; i++ ) {
        for ( int j = -1; j <= 1; j++ ) {
          if ( i == 0 && j == 0 )
            continue;
          touchFilterCell( expl_trans_map, { point[0] + i, point[1] + j }, point_queue,
                           frontier_cell_cluster );
        }
      }
    }
    if ( frontier_cell_cluster.size() > min_frontier_size ) {
      for ( const auto &cell : frontier_cell_cluster ) { new_frontier_cells.push_back( cell ); }
    }
  }
  if ( new_frontier_cells.empty() ) {
    ROS_WARN_STREAM(
        "After filtering small frontiers, no frontiers left -> using unfiltered frontiers" );
    return;
  }
  frontier_cells = new_frontier_cells;
}

void touchFilterCell( grid_map::Matrix &expl_trans_map, const grid_map::Index &index,
                      std::queue<grid_map::Index> &point_queue,
                      std::vector<grid_map::Index> &frontier_cell_cluster )
{
