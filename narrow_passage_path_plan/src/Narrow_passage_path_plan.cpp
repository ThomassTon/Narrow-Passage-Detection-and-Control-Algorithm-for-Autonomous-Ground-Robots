#include <Narrow_passage_path_plan.hpp>
...
NarrowPassageController::NarrowPassageController( ros::NodeHandle &nodeHandle ) : nh( nodeHandle )
{
  ...
  approach_point_sub = nh.subscribe("/narrow_passage_entrance", 1, &NarrowPassageController::approach_point_messageCallback, this );
...
bool NarrowPassageController::check_path_collision(const nav_msgs::Path &circle){
  ...
  if(get_elevation_map){
    for(int i=0; i< circle.poses.size()-1;i++){
      ...
      for ( grid_map::CircleIterator iterator( elevation_map, pos, 0.25 ); !iterator.isPastEnd(); ++iterator ) {
        const double &value = elevation_map.at( "elevation", *iterator );
        // std::cout<<"iter check circle"<<value<<"\n\n\n\n";
        v_b.push_back(value);
        if ( value > 0.5 && value != NAN ) {
