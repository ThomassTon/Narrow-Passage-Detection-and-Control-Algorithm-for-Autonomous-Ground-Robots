  ...
  : state(INACTIVE), stuck(new StuckDetector(nh_))
{
  ...
  mp_.inclination_speed_reduction_time_constant = 0.3;   //0.3
...
bool Controller::drivepath(const nav_msgs::Path& path)
{
  ...
  removeLegsInThePast();
  if(!legs.empty()) {
    ROS_INFO("[vehicle_controller] Received new path to goal point (x = %.2f, y = %.2f, ori = %.2f)", legs.back().p2.x, legs.back().p2.y, legs.back().p2.orientation);
    state = DRIVEPATH;
  }
  else {
...
//                    ". Original speed: " << legs[current].speed << ". Speed: " << speed);
  ...
  robot_control_state.setControlState(speed,  // speed    0.2
                                      desired_position,
                                      error_2_path,
                                      error_2_carrot,
                                      mp_.carrot_distance,
